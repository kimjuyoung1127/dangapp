...
| DANG-MAT-001 | matching/filter | `/home`, `/modes` | rewrite | In Progress | integration/e2e | High | `match_guardians_v2` + `useMatch.ts` aligned, `/modes` B2B status dashboard added (care/family live counts + retry + summary helper), Playwright route harness added and `@signed` PASS with storageState |
