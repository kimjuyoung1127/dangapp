...
Last Updated: 2026-03-04 (KST) — auto-synced by parity-cascade-sync
Owner Doc: `CLAUDE.md`
...
| Wave 1: schema foundation | Done | 고도화 완료 (Phase 1~3) + 리뷰 보강 패치 반영: Auth Sync 안정화, match_guardians_v2 JSONB 안전화, 신규 테이블 RLS/정책 적용, 공간 인덱스 보강. |
| Wave 2: onboarding rebuild | Done | /onboarding 구현 완료 (RHF+Zod+Supabase hook+upload). Type Modeling & Mapper 고도화 완료. |
| Wave 3: home matching/filter | InProgress | `useMatch.ts` -> `match_guardians_v2` 연동 완료. `/modes` 라우트 및 E2E 검증 잔여. |
